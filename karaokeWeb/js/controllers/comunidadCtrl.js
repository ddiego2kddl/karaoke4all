...
app.controller('muroComunidadCtrl', function ($scope, $http, comunidadService, sessionService, $q) {
	...
	$scope.goTo=function(id_user){
		...
		if (esFriend){
			aux.extraDesc="Dejar de seguir";
            aux.icon_follow="glyphicon glyphicon-eye-close";
        }
		else{
			aux.extraDesc="Seguir";
            aux.icon_follow="glyphicon glyphicon-eye-open";
        }
...
////////////////////////////////////////////////////
// ******** CONTROLLER DEL SIGUIENDO ******** //
////////////////////////////////////////////////////

app.controller('siguiendoComunidadCtrl', function ($scope, $http,comunidadService, sessionService) {	
    $scope.path = path;
    var init = function(){
		$scope.followers=[];
...
////////////////////////////////////////////////
app.controller('seguidoresComunidadCtrl', function ($scope, $http, comunidadService, sessionService, $q) {
	$scope.path = path;
...
////////////////////////////////////////////////////
app.controller('mensajesComunidadCtrl', function ($scope, $http,comunidadService, sessionService,$q,$window) {
	$scope.path = path;
	...
	var init= function(){
    	...
    	$q.all([mensajesRecepter,users,mensajesSender]).then(function(dataArray){
            ...
            $scope.mensajes[i].fecha = new Date($scope.mensajes[i].fecha);
            console.log($scope.fecha);
            }
           
            
             $scope.user2=$scope.mensajes[$scope.mensajes.length-1].user2;
            init2();
    	});
    ...
    var init2 = function(){
    	comunidadService.getMensajesbyIdBD2($scope.user2.id_user).then(function(mensajes){
    		$scope.mensajes2 = mensajes.data;
    		for (var i=0;i<$scope.mensajes2.length;i++){
    			if ($scope.mensajes2[i].id_sender == $scope.user2.id_user){
    				$scope.mensajes2[i].tipo = "conversTu";
                    $scope.mensajes2[i].icon = "glyphicon glyphicon-triangle-right icon-right"
                }
    			else{
    				$scope.mensajes2[i].tipo = "conversEl";
                    $scope.mensajes2[i].icon = "glyphicon glyphicon-triangle-left icon-left"
                }
    		}
    	})
    }
    ...
    $scope.sendChat = function(){
    	...
    	comunidadService.sendMensajeBD2($scope.textChat,$scope.user2.id_user).then(function(){
    		init();
    		$scope.textChat="";
    	})
    }
    	...
    	comunidadService.getMensajesbyIdBD().then(function(mensajes){
    		...
    		for (var i=0;i<$scope.mensajes.length;i++){
    			if ($scope.mensajes[i].id_sender == userGlobal.id_user){
                    ...
                    $scope.mensajes[i].icon = "glyphicon glyphicon-triangle-right icon-right"
                }
    			else{
    				$scope.mensajes[i].tipo = "conversEl";
                    $scope.mensajes[i].icon = "glyphicon glyphicon-triangle-left icon-left"
                }
    		}
    	})
    }
